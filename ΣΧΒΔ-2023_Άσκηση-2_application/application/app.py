...
def findTrips(x, a, b):
    ...
    packages_query = f"""SELECT DISTINCT tp.trip_package_id, tp.cost_per_person, tp.max_num_participants, COUNT(r.Reservation_id) AS reservations,
    ...
    for package_data in packages_data:
        ...
        if len(travel_guides) > 0:
            fullnamestr = ""
            i = 0
            for fullname in travel_guides:
                fullnamestr += f"{fullname[0]} {fullname[1]}"
                if i < len(travel_guides)-1:
                    fullnamestr += ", "
                i += 1
            package_data.insert(4, fullnamestr)
        else:
            package_data.insert(4, "")
        results.append(package_data)

    results.insert(0, ("cost_per_person", "max_num_participants", "reservations", "empty_seats", "guide_full_name", "trip_start", "trip_end"))
    return results
